...
class Lidar:
    ...
    Attributes:
        ...
        max_packets (int): The number of packets expected to be taken per ring (245).
        packet_size (int): The number of bytes per packet (47).
        start_byte (int):  The fixed start byte in accordance with the STL27L 
            communication protocol (0x54).
    ...
    def __init__(self) -> None:
        '''
        Initializes a Lidar object with hard-coded application-specific parameters.
        '''
        
        self.name        = 'STL27L'
        self.max_packets = 245		    # (921600b/s) / (10Hz) / (8b/B) / (47 B/packet)
        ...
        self.serial = serial.Serial()   # Creates (but does not open) a serial connection
...
def test_ring_capture(save: bool = False, verbose: bool = True) -> tuple[float, int]:
    ...
    polar_points: list[list[float]] = L1.capture_ring()
